...
class Smarty_Internal_Template extends Smarty_Internal_Data {
    ...
    */
    public function __construct($template_resource, $smarty, $_parent = null, $_cache_id = null, $_compile_id = null, $_caching = null, $_cache_lifetime = null)
    {
        $this->smarty = &$smarty; 
        // Smarty parameter
        $this->smarty->cache->data_id = $_cache_id === null ? $this->smarty->cache->data_id : $_cache_id;
        ...
        $this->smarty->cache->data_lifetime = $_cache_lifetime === null ?$this->smarty->cache->data_lifetime : $_cache_lifetime;
        ...
        if (!$this->resource_object->isEvaluated && ($this->caching == SMARTY_CACHING_LIFETIME_CURRENT || $this->caching == SMARTY_CACHING_LIFETIME_SAVED)) {
            $this->smarty->cache->data_resource_object = $this->smarty->loadCacheResource();
        } 
    } 
    ...
    */
    ...
    {
        return $this->cache->datad_filepath === null ?
        $this->cache->datad_filepath = ($this->resource_object->isEvaluated || !($this->caching == SMARTY_CACHING_LIFETIME_CURRENT || $this->caching == SMARTY_CACHING_LIFETIME_SAVED)) ? false : $this->cache->data_resource_object->getCachedFilepath($this) :
        $this->cache->datad_filepath;
    } 
    ...
    */
    ...
    {
        return $this->cache->datad_timestamp === null ?
        $this->cache->datad_timestamp = ($this->resource_object->isEvaluated || !($this->caching == SMARTY_CACHING_LIFETIME_CURRENT || $this->caching == SMARTY_CACHING_LIFETIME_SAVED)) ? false : $this->cache->data_resource_object->getCachedTimestamp($this) :
        $this->cache->datad_timestamp;
    } 
    ...
    */
    public function getCachedContent ()
    {
        ...
        $this->rendered_content = ($this->resource_object->isEvaluated || !($this->caching == SMARTY_CACHING_LIFETIME_CURRENT || $this->caching == SMARTY_CACHING_LIFETIME_SAVED)) ? false : $this->cache->data_resource_object->getCachedContents($this) :
    ...
    */
    public function writeCachedContent ($content)
    {
        ...
        $this->properties['cache_lifetime'] = $this->cache->data_lifetime;
        return $this->cache->data_resource_object->writeCachedContent($this, $this->createPropertyHeader(true) . $content);
    } 
    ...
    public function isCached ()
    {
        if ($this->isCached === null) {
            ...
            if (($this->caching == SMARTY_CACHING_LIFETIME_CURRENT || $this->caching == SMARTY_CACHING_LIFETIME_SAVED) && !$this->resource_object->isEvaluated && !$this->force_compile && !$this->force_cache) {
                ...
                if ($this->caching === SMARTY_CACHING_LIFETIME_SAVED || ($this->caching == SMARTY_CACHING_LIFETIME_CURRENT && (time() <= ($cachedTimestamp + $this->cache->data_lifetime) || $this->cache->data_lifetime < 0))) {
                    if ($this->smarty->debugging) {
                        Smarty_Internal_Debug::start_cache($this);
                    } 
                    $this->rendered_content = $this->cache->data_resource_object->getCachedContents($this);
                    if ($this->smarty->debugging) {
                        Smarty_Internal_Debug::end_cache($this);
                    } 
                    if ($this->cache->dataFileChecked) {
                        $this->isCached = true;
                        return $this->isCached;
                    } 
                    $this->cache->dataFileChecked = true;
    ...
    */
    public function parseResourceName($template_resource, &$resource_type, &$resource_name, &$resource_handler)
    {
        ...
        if ($resource_type != 'string') {
            $this->smarty->template_objects[crc32($this->template_resource . $this->cache->data_id . $this->compile_id)] = $this;
        } 
        return true;
    } 
