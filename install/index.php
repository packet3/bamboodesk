...
class td_install {
    ...
    private function step_3($params=array())
    {
        ...
        if ( $this->trellis->input['store_db'] )
        {
            ...
            if ( ! $cid = @mysqli_connect( $this->trellis->cache->data['install']['config']['db_host'] .':'. $this->trellis->cache->data['install']['config']['db_port'], $this->trellis->cache->data['install']['config']['db_user'], $this->trellis->cache->data['install']['config']['db_pass'] ) ) $this->step_2( array( 'error' => 'Could not connect to the MySQL Server. Please check to verify that your MySQL information is correct and try again.' ) );

            $mysqli_ver = mysqli_get_server_info($cid);

            if ( strpos( $mysqli_ver, '-' ) ) $mysqli_ver = substr( $mysqli_ver, 0, strpos( $mysqli_ver, '-' ) );

            if ( version_compare( $mysqli_ver, '4.1', '<' ) ) $this->step_2( array( 'error' => 'Sorry, Trellis Desk cannot be installed as it requires MySQL version 4.1 or later.' ) );

            if ( ! @mysqli_select_db($cid, $this->trellis->cache->data['install']['config']['db_name'] ) ) $this->step_2( array( 'error' => 'Could not connect to the MySQL Database. Please check that your MySQL credentails are correct and try again.' ) );
    ...
    private function install_other()
    {
        ...
        if ( ! $this->trellis->cache->data['install']['do_other'] )
        {
            ...
            if ( ! $cid = @mysqli_connect( $this->trellis->cache->data['install']['config']['db_host'] .':'. $this->trellis->cache->data['install']['config']['db_port'], $this->trellis->cache->data['install']['config']['db_user'], $this->trellis->cache->data['install']['config']['db_pass'] ) ) $this->step_2( array( 'error' => 'Could not connect to the MySQL Server.' ) );

            if ( ! @mysqli_select_db($cid, $this->trellis->cache->data['install']['config']['db_name'] ) ) $this->step_7( array( 'error' => 'Could not connect to the MySQL Database.' ) );
            ...
            {
                if ( ! @mysqli_query($cid, $sql_query ) ) $this->step_7( array( 'error' => 'An error encountered while trying to run the following SQL Query.<br /><br />'. $sql_query .'<br /><br />MySQL returned the following error.<br /><br />'. mysqli_error($cid) .'<br /><br />'. mysqli_errno($cid) ) );
            }
